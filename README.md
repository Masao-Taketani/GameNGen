...
> [!NOTE]
> If you would just like to check the behavior of the trained model, skip to [Run autoregressive inference](#run-autoregressive-inference). The necessary steps to run the inference code is written there.
...
## Modifications
Here are the list of overall modifications from [arnaudstiegler/gameNgen-repro](https://github.com/arnaudstiegler/gameNgen-repro/tree/main). For more details, please look at each item listed in `To Do`.
...
- unet_model_folder (str): Specify your trained U-net folder. Download the trained diffusion model if not specified
- vae_ft_model_folder (str): Specify your finetuned VAE folder. Download the trained VAE model if not specified
